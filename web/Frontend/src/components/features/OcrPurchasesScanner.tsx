...
interface OcrPurchasesScannerProps {
...
export default function OcrPurchasesScanner({ availableProducts, onApply, onClose, initialData }: OcrPurchasesScannerProps) {
  ...
  const parseOcrText = (text: string) => {
    ...
    // Parse Invoice ID
    const idRegex = /(?:so hoa don|so hd|ma hd|hd|invoice\s*no|invoice|so\s*hd|noa\s*don|so\s*noa\s*don|don\s*nhap|ma\s*nhap)\s*[:.-]?\s*([^\s,;]+)/i;
    let foundId = '';
    
    // Parse Date
    const dateRegex = /(?:ngay|date)\s*[:.-]?\s*([\d\/\.-]+)/i;
    let foundDate = '';

    // Parse Partner details (Supplier / NCC name)
    const customerNameRegex = /\b(?:khách\s+[hàb]àng|khach\s+[hab]ang|customer|tên\s+khách|ten\s+khach|người\s+mua|nguoi\s+mua|\bkh(?=\s*[:.-]|\s+)|nhà\s+cung\s+[cấa]p|nha\s+cung\s+[ca]p|ncc)\s*[:.-]?\s*([^\n\r]+)/i;
    ...
    const paidRegex = /(?:khach hang thanh toan|khach thanh toan|da tra|thanh toan|paid|da tt|da thanh toan)\s*[:.-]?\s*([\d.,\s]+)/i;
    ...
    for (const line of lines) {
      ...
      // Create normalized line for accent-insensitive matching
      const cleanLineForMatch = trimmed.toLowerCase()
                                       .normalize('NFD')
                                       .replace(/[\u0300-\u036f]/g, '')
                                       .replace(/[đĐ]/g, 'd');

      if (!foundId) {
        const idMatch = cleanLineForMatch.match(idRegex);
        if (idMatch) {
          foundId = idMatch[1].replace(/[^\d]/g, ''); // Extract only digits for invoice ID
        }
      }

      if (!foundDate) {
        const dateMatch = cleanLineForMatch.match(dateRegex);
        if (dateMatch) {
      ...
      const paidMatch = cleanLineForMatch.match(paidRegex);
      if (paidMatch) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!inTable && (
        ...
        cleanLine.includes('ten san pham') ||
        cleanLine.includes('thanh tien') ||
        cleanLine.includes('don gia') ||
        cleanLine.includes('dvt') ||
        cleanLine.includes('stt') ||
        /^[|]*\s*1\s+/.test(cleanLine)
      )) {
        inTable = true;
        if (/^[|]*\s*1\s+/.test(cleanLine)) {
          // If it starts with item 1, don't skip this line!
        } else {
      ...
      if (
        ...
        cleanLine.includes('tong tien') ||
        cleanLine.includes('thanh toan') ||
        cleanLine.includes('da tt') ||
        cleanLine.includes('con lai') ||
        cleanLine.includes('con tai')
      ) {
        inTable = false;
        break; // Stop parsing altogether
      }

      if (!inTable) {
        continue; // Skip lines that are not inside the table
      }
  ...
  const handleRowChange = (id: string, field: keyof ParsedItem, value: any) => {
    setParsedItems(prev => prev.map(item => {
      if (item.id === id) {
        ...
        // If product mapped is changed, auto-update the unit price (Purchases uses giaNhapHienTai)
        if (field === 'mappedProductId') {
          const prodId = parseInt(value);
          const matchedProd = availableProducts.find(p => p.id === prodId);
          if (matchedProd) {
            updated.price = matchedProd.giaNhapHienTai;
          }
        }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-[#0c1322] border-r border-white/5 overflow-hidden">
      ...
      <div className="p-4 border-b border-white/5 bg-[#0f192b] flex justify-between items-center">
        <div>
          <h3 className="text-xs font-bold text-white flex items-center gap-1.5">
            ...
            OCR Trích xuất Hóa đơn (Nhập hàng)
          </h3>
          <p className="text-[9px] text-on-surface-variant">Tải ảnh hóa đơn đỏ/giao hàng lên để trích xuất nguyên liệu tự động.</p>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="flex-1 flex min-h-0 divide-x divide-white/5 overflow-hidden">
        ...
        <div className="w-1/2 flex flex-col p-4 space-y-4 overflow-y-auto min-h-0">
          ...
          {!ocr.imagePreview ? (
            <div
              ...
              onClick={() => document.getElementById('purchases-file-input')?.click()}
        ...
        <div className="w-1/2 flex flex-col p-4 space-y-4 overflow-y-auto min-h-0">
          <label className="text-[10px] uppercase font-bold text-on-surface-variant tracking-wider">Kết quả phân tích (Nhập hàng):</label>
          ...
          {parsedItems.length === 0 && !ocr.isScanning ? (
            <div className="flex-1 border border-white/5 rounded-xl flex flex-col items-center justify-center text-center p-6 text-on-surface-variant bg-black/5">
              ...
              <p className="text-[9px] mt-1 max-w-[200px]">Sau khi quét hóa đơn thành công, các nguyên liệu trích xuất được sẽ hiển thị ở đây.</p>
            </div>
          ) : (
            <div className="flex-1 flex flex-col space-y-4 min-h-0">
              <div className="grid grid-cols-2 gap-3 bg-white/[0.02] border border-white/5 p-3 rounded-xl">
                ...
                <div>
                  <label className="block text-[9px] text-on-surface-variant mb-1 font-semibold">Nhà cung cấp / Đối tác:</label>
                  <input
                    type="text"
                    value={customerName}
                ...
                <div className="col-span-2">
                  <label className="block text-[9px] text-on-surface-variant mb-1 font-semibold">Tiền đã trả NCC:</label>
                  <input
                    type="number"
                    value={paidAmount || ''}
              ...
              <div className="flex-1 flex flex-col border border-white/5 rounded-xl overflow-hidden min-h-[180px]">
                <div className="bg-[#0f192b] px-3 py-2 flex justify-between items-center border-b border-white/5">
                  <span className="text-[9px] uppercase font-bold text-white">Nguyên liệu trích xuất ({parsedItems.length})</span>
                  <button
                    type="button"
                        ...
                        <div className="relative">
                          <div className="relative">
                            <input
                              ...
                              placeholder={
                                activeDropdownId === item.id
                                  ? (matchedProd ? matchedProd.tenSanPham : 'Gõ để tìm kiếm nguyên liệu...')
                                  : '-- Chọn nguyên liệu liên kết --'
                              }
                              onFocus={() => {
                            ...
                            <div className="absolute left-0 right-0 mt-1 bg-[#0f192b] border border-white/10 rounded-lg shadow-xl max-h-40 overflow-y-auto z-50 divide-y divide-white/[0.02]">
                              ...
                              {availableProducts
                                ...
                                .map(p => (
                                  ...
                                  >
                                    {p.tenSanPham} ({formatVND(p.giaNhapHienTai)})
                                  </div>
                                ))}
                            </div>
                          )}
