...
export const reportService = {
  ...
  async getProductImports(params?: { startDate?: string; endDate?: string }): Promise<any[]> {
    const query = new URLSearchParams();
    if (params?.startDate) query.append('fromDate', params.startDate);
    if (params?.endDate) query.append('toDate', params.endDate);
    const queryString = query.toString();
    return api.get<any[]>(`/bao-cao/san-pham-nhap${queryString ? `?${queryString}` : ''}`);
  },

  async getProductImportsStats(params?: { startDate?: string; endDate?: string }): Promise<any> {
    const query = new URLSearchParams();
    if (params?.startDate) query.append('fromDate', params.startDate);
    if (params?.endDate) query.append('toDate', params.endDate);
    const queryString = query.toString();
    return api.get<any>(`/bao-cao/san-pham-nhap/stats${queryString ? `?${queryString}` : ''}`);
  },
};
